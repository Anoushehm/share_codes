...
*      DAD annual files are merged to DAD key file based on three variables: 
*      "dad_transaction_id", "submitting_prov_code", and "fiscal_year". The 
...
*      "_all_hospdeath" --> indicating that the dataset has dummy variables for 
...
*    through in this do-file; A dataset matures from being a Raw file into a 
*    Master file and finally becomes a Use file. However, these are not the only 
...
* Set directory
cd P:\Proj-22_FFS_10058_Plante_Sanjoy\WORK\Anousheh
...
program define harm_keyfile_bs	
  * This program renames and recodes variables in the CanCHEC Bootstrap Keyfile to 
  * be similar to what exists in other datasets in CanCHEC. 
...
program define harm_keyfile_cmdbonly		
  * This program renames and recodes variables in the CanCHEC CMDBonly Keyfile to
  * be similar to what exists in other datasets in CanCHEC. 
  ...
  * Dropping unnecessary characters from age variable (CMDBonly)            ****
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - *
     ...
     age variable is saved as "5049". Because of this, "50" was dropped from the 
     beginning of the age values to get the age of the deceased.
  * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */  
  ...
  * Census   
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - *
     In CMDBonly Keyfile and CVSD, the sex variable is saved as "1" for males and
     "2" for females. However, in Census 2006, female is coded as 1 and male as 2.
     To be consistent across all datasets, the sex variables in CVSD and 
     CMDBonly key file are changed to be similar to the Census' sex variable.
  * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */    
...
program define reduce_dad
  ...
  * keeps those that are more likely to be used in the final cohort. The reduced 
  * datasets are saved separately for each year.
  ...
  forvalues year = 2006/2016 { 
    ...
    * Keep variables we need
    keep dad_transaction_id submitting_prov_code inst_code fiscal_year       ///
      health_card_prov_code patient_postal_code responsibility_for_payment   ///
      age_code age_units gender_code total_los_days acute_los_days           ///
      admission_date xfer_from_inst_type_code discharge_date alc_los_days    ///
      xfer_from_inst_code admission_category xfer_to_inst_type_code          ///
      xfer_to_inst_code inst_type_code diag*         
...
program define append_dad	
  ...
  * result with DAD keyfile. This merge enables linking DAD to the Master key file
  * in Step 5.
  ...
  *  Merge results                                                          ****
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - *
   - MASTER FILE:
       The number of observations equals the number of abstracts recorded in DAD 
       from 2006 to 2016. This is the same as the number of hospitalizations 
       that happened to the CanCHEC 2006 cohort in these years.
     
   - USING FILE:
       The number of observations equals the number of abstracts recorded in DAD
       from 2000 to 2016. This is the same as the number of hospitalizations
       that happened to the CanCHEC 2006 cohort in these years.
  ...
  *  Note on age variable in DAD                                            ****
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - *
     ...
     information:
       ...
       2. Age Code: is the value denoting how the patient's age is measured. For
          example, "Y" indicates that age is measured by year.
  ...
  *  How to use the saved DAD Master file                                   ****
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - *
     ...
     to the Master Keyfile by uniqid, so that we include hospitalization records 
     in the final cohort.
  * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */	
end		
...
program define append_cvsd
  * This program appends harmonized CVSD datasets (2006 to 2016) resulting in the 
  * previous step.
  ...
  *  Note on age variables in CVSD                                          ****
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - *
     ...
     1. Age_value: Actual age of deceased reported in time units of either 
  	  minutes, hours, days, months, or years (as specified by the age code.)
  ...
  *  Note on sex variable in CanCHEC                                        ****  
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - *
     ...
     female and 2 for male, this is the other way around in CVSD. So, we changed
     the CVSD's sex variable to match what we have in the Census.
  * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
...
program define merge_cvsd_multi
  ...
  * The reason to do so is that in order to identify deaths due to substance use
  * POISONING, we need to consider variables (i.e., contributing cause,) which 
  * only exists in the Multiple Cause dataset. 
  ...
  * information for each death registered in the main CVSD files, which MUST be 
  * used to identify SPECIFIC deaths, such as deaths due to substance use 
  * poisoning. That is, the contributing cause variables are not useful to 
  * identifying EVERY TYPE OF DAETH. For example, we don't need to consider them 
  * when identifying deaths due to non-poisoning substance use causes. 
  ...
  *  How CVSD main file relates to Multi Cause file                         ****
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - *
     ...
     records for contributing causes of death for those observations missing in
     the Multi Cause file.
  * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  *  Merge results                                                          ****	
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - *
   - MASTER FILE:
       The number of observations in the Master File equals the number of deaths  
       in Multi Casue file from 2000 to 2016.
...
program define prep_cens
  ...
  * Open Census 2006 raw file
  ...
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - *
     Note that the number of observations in this file is more than the number of 
     CanCHEC 2006 cohort, which equals the number of observations in Master 
     Keyfile. This is because not every respondent of the Census (long-form) is 
     eligible to be included in the CanCHEC cohort.
  * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  *  Race and ethnicity in Census                                           **** 
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - *
  For dvismin == 14, the frequency in Census 2006 is zero, and therefore it is  
  ...
  * Generate education variable using the Highest Certificate, Diploma or Degree
  * variable in Census
  generate edu = .
...
program define create_cohort_nat
  ...
  * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - *
  MASTER FILE:
    The number of observations equals the number of people in the CanCHEC 2006 
    cohort.
  
    The number of matched observations equals the number of people in CanCHEC 
    2006 who died between 2006 to 2016.
    
    The number of not matched equals the number of people in CanCHEC 2006 who did 
    not die between 2006 to 2016.
    
  USING FILE:  
    The number of observations equals the number of deaths recorded for people in 
    the CanCHEC 2006 cohort from 2006	to 2016.
    
    The number of matched observations equals the number of deaths happened to 
    people included in CanCHEC 2006 between 2006 to 2016.
    
    The number of not matched observations equals the number of deaths happened
    to all Canadians who are not included in the CanCHEC 2006 between 2006 to 
    2016.
  	
  MERGE RESULT:
    matched 
      equals the number of people in CanCHEC 2006 who died between 2006 to 2016.
  	
    not matched from master 
      equals the number of people in CanCHEC 2006 who did not die between 2006 to 
  	2016.
  	
    not matched from using = 
  	
  Since  there are no observations in our using 
  	file that have missing values for the three key variables. So, using m:1
  	did not make any problem.
  * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

  * Merge with CMDBonly file to add deaths that were not captured by CVSD
  merge 1:1 uniqid using ".\Data\canchec2006_key_cmdbonly_HARM.dta", nogen update  ///
    replace keep(1 4)
  ...
  USING FILE:  
    The number of observations equals the number of deaths recorded in CMDB for 
    people in the CanCHEC 2006 cohort from 2006 to 2016. These deaths are not 
    captured by CVSD.
  	
  MERGE RESULT:
  	matched - missing updated = 
   	matched - nonmissing conflict = 
  	mathced - not updated = 
  	not matched from master = 
  	not matched from using = 
  * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */	
  ...
  * Merge with DAD (2006-16)
  ...
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - *
  Master file has  
  	(== number of people in the CanCHEC 2006 cohort.)
  Using file has .
  	(== number of hospitalizations recorded for people in the CanCHEC 2006 cohort
  		from 2006 to 2016.)
  	
  MERGE RESULT:
  	matched =  	
  	not matched from master = 
  		(== number of people in CanCHEC 2006 cohort who have no hospitalization 
  			records in DAD from 2006 to 2016.)
...
program define create_cohort_pr
  ...
  * Canadians (CanCHEC2006_cohort_can_06to16_e_USE) created in the previous step.
  ...
  * Identify Saskatchewanian abstracts (criteria: SK health card OR postal code
  * starts with S)
  ...
  foreach category in alc opi can cnsdep coc cnsstim unkmult other {
    ...
    * on contributing causes of death
    forvalues n = 1/20 {
      ...
      * criteria      
      foreach icd of local pois_contr_`category'{
...
program define label_vars
  ...
  * Label variables
  label variable  uniqid            "Unique ID"
  label variable  sex_death         "Sex (CVSD)"
  label variable  race              "Race"
  label variable  edu               "Education"
  label variable  emp               "Employment status"
  label variable  sex_hosp          "Sex (DAD)"
  label variable  hosp_e            "Hospitalization event"
  label variable  hosp_p            "Person who experienced hospitalization"
  label variable  death_e           "Death event"
  label variable  death_p           "Person who died"
  label variable  hosp_suh_e        "Hospitalization event due to substance use"
  label variable  hosp_suh_p        "Person who experienced hospitalization due to substance use"
  label variable  hosp_alc_e        "Hospitalization event due to alcohol"
  label variable  hosp_alc_p        "Person who experienced hospitalization due to alcohol"
  label variable  hosp_opi_e        "Hospitalization event due to opioid"
  label variable  hosp_opi_p        "Person who experienced hospitalization due to opioid"
  label variable  hosp_can_e        "Hospitalization event due to cannabis"
  label variable  hosp_can_p        "Person who experienced hospitalization due to cannabis"
  label variable  hosp_cnsdep_e     "Hospitalization event due to CNS depressants"
  label variable  hosp_cnsdep_p     "Person who experienced hospitalization due to other CNS depressants"
  label variable  hosp_coc_e        "Hospitalization event due to cocaine"
  label variable  hosp_coc_p        "Person who experienced hospitalization due to cocaine"
  label variable  hosp_cnsstim_e    "Hospitalization event due to CNS stimulants"
  label variable  hosp_cnsstim_p    "Person who experienced hospitalization due to other CNS stimulants"
  label variable  hosp_unkmult_e    "Hospitalization event due to unknown and multiple substances"
  label variable  hosp_unkmult_p    "Person who experienced hospitalization due to unknown and multiple substances"
  label variable  hosp_other_e      "Hospitalization event due to other substances"
  label variable  hosp_other_p      "Person who experienced hospitalization due to other substances"
  label variable  death_suh_e       "Death due to substance use"
  label variable  death_suh_p       "Person who died due to substance use"
  label variable  death_alc_e       "Death event due to alcohol"
  label variable  death_alc_p       "Person who died due to alcohol"
  label variable  death_opi_e       "Death event due to opioid"
  label variable  death_opi_p       "Person who died due to opioid"
  label variable  death_can_e       "Death event due to cannabis"
  label variable  death_can_p       "Person who died due to cannabis"
  label variable  death_cnsdep_e    "Death event due to CNS depressants"
  label variable  death_cnsdep_p    "Person who died due to other CNS depressants"
  label variable  death_coc_e       "Death event due to cocaine"
  label variable  death_coc_p       "Person who died due to cocaine"
  label variable  death_cnsstim_e   "Death event due to CNS stimulants"
  label variable  death_cnsstim_p   "Person who died due to other CNS stimulants"
  label variable  death_unkmult_e   "Death event due to unknown and multiple substances"
  label variable  death_unkmult_p   "Person who died due to unknown and multiple substances"
  label variable  death_other_e     "Death event due to other substances"
  label variable  death_other_p     "Person who died due to other substances"
  label variable  w_cohort          "CanCHEC 2006 weight"
  label variable  rufg              "Rural, Urban classification"
  
  * Label sex variables' values
  label define sexfull                                                       ///
    1  "Female"                                                              ///
    2  "Male", replace          
  label values sex_death sexfull
  
  * Label sex variable's values
  label define sexfull                                                       ///
    0  "Other"                                                               ///
    1  "Female"                                                              ///
    2  "Male", replace  
 ...
 * Label the generated education variable
  ...
  * Label all derived dummy indicators
  label define dummy                                                         ///
    0  "No"                                                                  ///
	  1  "Yes", replace
